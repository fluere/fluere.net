 ...
 */
function(t){"use strict";var e=t(window),n=t("html"),o=t("header.page-header"),i=t("div.content"),a=n.hasClass("mobile"),r=!1,s=function(){h(),d(),u(),f(),p(".page-header, .page-footer"),l(),r=!0},c=function(){i.addClass("is-loading"),window.scrollTo(0,o.outerHeight())},l=function(){t("#main-content").css("minHeight",e.height()),r&&window.scrollTo(0,o.outerHeight()),p("#main-content"),g(),i.find("code[data-gist-id]").length&&window.gistEmbed(),"ga"in window&&window.ga("send","pageview",window.location.pathname+window.location.search),i.filter(".is-loading").removeClass("is-loading").find(".archive__title, .archive__item, .entry__header, .entry__body>*, .entry__comment, .pagination, .return-home").fadeTo(0,0).each(function(e){t(this).stop(!0,!0).delay(70*(e+1)+100).fadeTo(300,1)})},h=function(){e.on("load resize"+(a?"orientationchange scroll":""),function(){o.css("minHeight",e.height())}).trigger("resize")},d=function(){var e='a:not([target]):not([href^="#"])';a?t(e).not('[href*="#"]').each(function(){t(this).attr("href",t(this).attr("href")+"#main-content")}):(t.pjax({area:"#main-content",link:e,scrollTop:!1,scrollLeft:!1}),t(document).on("pjax:fetch",c).on("pjax:DOMContentLoaded",l))},u=function(){a||n.niceScroll({cursoropacitymax:.4,cursorwidth:7,mousescrollstep:20})},f=function(){var n=t("div.background");n.is(":visible")&&e.on("scroll.swapbg",function(){var t=e.height(),o=e.scrollTop();t>=o?n.css("opacity",Math.max(0,1-o/t)):n.css("opacity",0)}).trigger("scroll.swapbg")},p=function(e){t(e).find("a").filter('[href^="http"]:not([href*="'+location.hostname+'"]):not([target])').attr("target","_blank").end().filter('[href^="#"]:not([href="#"]):not([rel="footnote"])').smoothScroll().end().filter('[rel="footnote"]').smoothScroll({highlight:!0}).end().filter(".popup").popup()},g=function(){var e=location.pathname.split("/");"tag"!==e[1]||e.length<3||(t("title").text(t("title").text().replace(/^archive/,"#"+e[2])),t("h1.archive__title").text("#"+e[2]))};t(document).ready(s),e.on("load",function(){t("html").addClass("is-ready")})}(window.jQuery),/*!
